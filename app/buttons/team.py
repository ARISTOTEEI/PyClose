    ...
    async def on_button_click(self,inter:dis.MessageInteraction):
        ...
        if button == "team":
            ...
            row.add_button(
                ...
                disabled=next((True for x in members if x.pos == 1 and x.team == team),False)
            )
            row.add_button(
                style=dis.ButtonStyle.grey,
                custom_id=f"pos.2.{team}.{close_id}",
                emoji=self.bot.settings.emojis.Onion,
                disabled=next((True for x in members if x.pos == 2 and x.team == team),False)
            )
            row.add_button(
                style=dis.ButtonStyle.grey,
                custom_id=f"pos.3.{team}.{close_id}",
                emoji=self.bot.settings.emojis.Security,
                disabled=next((True for x in members if x.pos == 3 and x.team == team),False)
            )
            row.add_button(
                style=dis.ButtonStyle.grey,
                custom_id=f"pos.4.{team}.{close_id}",
                emoji=self.bot.settings.emojis.Conhand,
                disabled=next((True for x in members if x.pos == 4 and x.team == team),False)
            )
            row.add_button(
                style=dis.ButtonStyle.grey,
                custom_id=f"pos.5.{team}.{close_id}",
                emoji=self.bot.settings.emojis.Conhands,
                disabled = next((True for x in members if x.pos == 5 and x.team == team),False)
            )
