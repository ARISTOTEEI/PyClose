...
class CloseWinnerSelect(Cog):
    ...
    async def create_user(self, bot: CloseBot, discord_id: int) -> UserSchema:
        await bot.clm.create_user(discord_id)
        user = await bot.clm.get_user(discord_id)
        return user
            ...
            if inter.guild.get_role(self.bot.settings.roles.closemod) in inter.author.roles:
                ...
                if close.creator == inter.author.id:
                    ...
                    for member in members:
                        ...
                        user = user if user != None else await self.create_user(self.bot, member.discord_id)
                    ...
                    embed = dis.Embed.from_dict(
                        {
                            "title": "Dota 2 ・ Результаты  ᅠ  ᅠ  ᅠ  ᅠ  ᅠ  ᅠ",
                            "color": 3092790,
                            "fields": [
                                {
                                    "name": f"{'🏆' if team_win == "dark" else ''} Силы тьмы",
                                ...
                                {
                                    "name": f"{'🏆' if team_win == "light" else ''} Силы света",
                    ...
                    channel = inter.guild.get_channel(
                        self.bot.settings.channels.win_channel)
